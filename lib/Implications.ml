...
(* Recursively pulls our \exists on LHS *)
let rec deconjunctivizer_lhs form varset =
  ...
  | Exists (v, body) ->
      ...
      else deconjunctivizer_lhs body (VS.add v varset)
  ...
  | And (a, b) ->
      let a_new, varset = deconjunctivizer_lhs a varset in
      let b_new, varset = deconjunctivizer_lhs b varset in
      (And (a_new, b_new), varset)
  | _ -> (form, varset)
...
let rec deconjunctivizer_rhs form varset =
  ...
  | Implies (lhs, rhs) ->
      ...
      let lhs_new, varset = deconjunctivizer_lhs lhs varset in
      let rhs_new, varset = deconjunctivizer_rhs rhs varset in
      ( List.map
          (fun rhs_partic ->
            match rhs_partic with
            | Implies (a, b) -> Implies (And (lhs_new, a), b)
            | _ -> Implies (lhs_new, rhs_partic))
          rhs_new,
        varset )
  | And (a, b) ->
...
let deconjunctivizer pre post =
  ...
  fst (deconjunctivizer_rhs form (free_vars form VS.empty))
  ...
  let no_hole_verify form important_variables =
    ...
    if not (List.mem form (List.map (fun a -> a.form) !file_logger)) then (
      ...
      if kid_pid = 0 then (
        ...
        (* If it makes sense to do so, discharge a backup sygus query as well. *)
        has_exists form
      then (
        (* Set up the file and record in the structure. *)
               ...
               if snd !a = Unix.WEXITED 0 && snd !b = Unix.WEXITED 0 then
                 ...
                 if result = "unsat" then (
                   ...
                   if result = "(" then (
                     i := 2;
                     out := true)
                   else if result = "infeasible" then (
                     i := i + 1)
                   else i := 2
...
module NoHoleSimpleImplicatorZ3 () : ImplicationHandler = struct
  ...
  let implies pre post =
    List.fold_left
      (fun lazy_boolean form ->
        lazy (Lazy.force lazy_boolean && Lazy.force (base_verif form VS.empty)))
      (lazy true)
