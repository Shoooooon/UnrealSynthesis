...
open Variable
open Program
module CheckerSet = Set.Make (Checker)
...
let rec build_wpc_proof (trip : triple_no_pre) =
  ...
  | Numeric (ITE (b, n1, n2)) ->
      let else_hyp  =
        build_wpc_proof { prog = Numeric n2; post = trip.post }
      in
      let then_hyp  =
        build_wpc_proof { prog = Numeric n1; post = trip.post }
      in
      let guard_hyp  =
        build_wpc_proof
          {
  ...
  | Stmt (Seq (s1, s2)) ->
      let right_hyp =
        build_wpc_proof { prog = Stmt s2; post = trip.post }
      in
      let left_hyp =
        build_wpc_proof
  ...
  | Stmt (ITE (b, s1, s2)) ->
      ...
      in
      let then_hyp =
        build_wpc_proof { prog = Stmt s1; post = trip.post }
      in
      let guard_hyp =
        build_wpc_proof
          {
  ...
  | Stmt (While (b, inv, s)) ->
      let body_hyp =
        build_wpc_proof { prog = Stmt s; post = inv }
      in
      let guard_hyp_raw =
        build_wpc_proof
          {
...
let prove (trip : triple) =
  let strongest =
    build_wpc_proof { prog = trip.prog; post = trip.post }
  in
